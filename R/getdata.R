...
#
if(imwd$type == "wavelet") stop(
"Decomposition  not performed using the Non-Decimated Wavelet Transfrom"
)
Csize <- 2.^#m <- matrix(0, nrow = Csize, ncol = Csize)
(imwd$nlevels)
...
######################################################
msub1 <- #
matrix(imwd[[nm]], nrow = LengthD, ncol = LengthD)
...
#####################################################
sel <- #
seq(from = (1. - firstD), length = ndata)
nm <- lt.to.name(level - 1., "CD")
#
#########################################################
# Extract DC for this level
