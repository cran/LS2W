...
"probability" && policy != "fdr" && policy != "LSuniversal")
...
if(verbose == TRUE)
cat("Threshold for level: ", levels[
i], " is ", thresh[i], "\n")
}
}
}
...
noise.level <- sqrt(dev(c(x[[lt.to.name(levels[nthresh], "CD")]], x[[lt.to.name(levels[nthresh], "DC")]], x[[lt.to.name(levels[nthresh], "DD")]])))
minit <- n
dinit <- d
...
thinit <- qnorm(1. - (Q * minit)/(
2. * n)) * noise.level
minit1 <- length(dinit1[abs(dinit1) >=
thinit])
...
x[[lt.to.name(levels[i], "DC")]],
x[[lt.to.name(levels[i], "DD")]])
thresh[i] <- quantile(abs(d), prob = value[
i])
if(verbose == TRUE)
cat("Threshold for level: ", levels[
i], " is ", thresh[i], "\n")
